...
2. Write a method called **indexOfMax** that takes an array of integers and returns the index of the largest element.
     ...
     * Why or why not?
```
Although posssible it is not a good way to write this code because
enhanced "for" loops don't loop through the index but rather it goes
through the elements meaning it would not be able to return the index
with the largest value without having to write unecessary code
```
